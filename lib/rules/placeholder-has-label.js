...
function placeholderHasLabel($, content) {
    return new validator_1.default({
        ...
        isInvalid: function ($elem, attrs) {
            var placeholderAttrs = attrs ? attrs.filter(function (attr) { return /placeholder/gi.test(attr); }) : [];
            var idAttrs = attrs ? attrs.filter(function (attr) { return /id/gi.test(attr); }) : [];
