...
function mouseEventsWithoutKeyEvents($, content) {
    return new validator_1.default({
        ...
        isInvalid: function ($elem, attrs, events) {
            var mouseEvents = events ? events.filter(function (attr) { return /mouse/gi.test(attr); }) : [];
            var blurAndFocusEvents = events ? events.filter(function (attr) { return /blur|focus/gi.test(attr); }) : [];
